...
/*   Updated: 2023/04/12 16:21:10 by pguranda         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		|| req.get_cmd () == "NAMES")
	{
		...
		while (true)
		{
			...
			if (req.get_cmd () == "JOIN")
			{
				...
				send_message(":" + req.get_user()->get_nickname() + "!" + req.get_user()->get_name() + "@" + SERVER_NAME + " JOIN " + channelName, req.get_user()->get_fd());
			}
